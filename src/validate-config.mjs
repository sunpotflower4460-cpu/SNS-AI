...
export function validateConfig(config) {
  const errors = [];
  const modes = new Set(['auto', 'approval', 'manual', 'pause']);
  ...
  for (const [id, account] of Object.entries(config.accounts || {})) {
    ...
    for (const time of account.schedule?.times || []) {
      if (!validateTimeString(time)) errors.push(`${id}: invalid schedule time "${time}"`);
    }
    ...
    if (account.enabled && ['auto', 'approval'].includes(account.mode)) {
      ...
      if (account.platform === 'instagram') {
        const strategy = account.media?.strategy || 'none';
        if (strategy === 'none') errors.push(`${id}: Instagram autonomous mode requires media strategy`);
        if (strategy === 'pool' && !(account.media?.urls || []).filter(Boolean).length) {
          errors.push(`${id}: Instagram media pool is empty`);
        }
        if (['fixed', 'external'].includes(strategy) && !account.media?.url) {
          errors.push(`${id}: Instagram media.${strategy} requires media.url`);
        }
        if (strategy === 'endpoint' && !/^https:\/\//i.test(account.media?.endpoint || '')) {
          errors.push(`${id}: Instagram media.endpoint requires an HTTPS endpoint`);
        }
      }
    }
  }

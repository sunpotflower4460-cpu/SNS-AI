...
async function compactFile(path, retentionDays, dateKey, type, archive) {
  ...
  for (const raw of lines) {
    ...
    try { row = JSON.parse(raw); }
      ...
      quarantine.push({ quarantinedAt: new Date().toISOString(), file: path.replace(`${ROOT}/`, ''), raw: raw.slice(0, 4000) });
      continue;
    }
...
export async function runMaintenance() {
  ...
  const settings = config.defaults?.maintenance || {};
  ...
  for (const [relative, retentionKey, dateKey, type] of SPECS) {
    ...
    const result = await compactFile(path, Number(settings[retentionKey] ?? 180), dateKey, type, archive);
  ...
  if (quarantined.length) {
    ...
    const cutoff = Date.now() - Number(settings.quarantineRetentionDays ?? 30) * 86_400_000;

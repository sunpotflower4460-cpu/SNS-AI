...
11. OpenAI Video APIによる縦Reel動画生成
12. 生成画像/動画thumbnailのModeration + 視覚QA
...
17. 投稿後メトリクスの時間別収集
18. アカウント自身のbaselineとの相対評価
19. 十分なbaselineがある場合の極端な反応異常検知と一時AUTOブレーキ
20. ブレーキcooldown後の自動再開
21. rolling windowでのstrategy更新
22. experiment winner判定と次experiment開始
23. current / weekly report生成
24. Web参照URL・判断理由・media QA・media経路・errorの監査記録
25. Circuit BreakerによるAPI障害時の自動休止/復帰
26. API種類別の日次利用hard cap（画像/動画生成を含む）
27. token期限metadataの警告
28. Workflow失敗Issueの作成・復旧Close
29. stale approval Issueの自動失効
30. data dedupe / retention / archive / quarantine
31. 古い生成media assetの削除
32. official X / Instagram policy情報の定期監視とreview Issue作成
33. PR/main CI・全source構文・全workflow YAML・secret scan
...
6. API提供者の障害・料金変更・サービス停止そのもの
7. OpenAI側で対象image/video modelの利用権限が付与されていない場合のplatform-side access enablement

静止画と標準的な短尺Reel動画の生成自体は内蔵済みで、外部media serviceは必須ではありません。外部endpointは、独自素材検索・独自生成基盤・private repository用CDNなどを使いたい場合の任意拡張です。

...
**現在このリポジトリはManual-Onlyでロックされています**（[`docs/MANUAL_ONLY_MODE.md`](MANUAL_ONLY_MODE.md)）。以下は「アカウントが有効化され、かつManual-Onlyが別途レビューされ解除された後」に無人実行できる設計上の範囲を定義したものであり、現在の実際の挙動ではありません。現在は全てのSNS操作workflowが`workflow_dispatch`のみで、schedule triggerは付いていません — 「1. 定刻起動」を含め、下記のどの項目も人間が都度Actionsを手動実行するまで動きません。責任境界（何を自動化してよいか/してはいけないか）自体は現在も有効な設計方針です。

## 完全自動で回す範囲（設計）

SNS-AIは、アカウントが有効化され必要な認証が揃い、かつManual-Onlyが別途レビューされ解除された後、次を無人実行できるよう設計されています。
...
Manual-Onlyが有効な間はここまでが完了範囲です。`8. 問題なければ auto`自体、`account-control.yml`経由では拒否されます。Manual-Onlyを別途レビューして解除した場合のみ、健康監視・調査・生成・media QA・投稿・分析・安全ブレーキ・学習・実験・報告・保守が自動で循環する設計です。

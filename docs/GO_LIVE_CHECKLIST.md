...
- workflowは**手動実行のみ**（`engagement-scheduled.yml` に `schedule:` は未設定。自動polling再開は別途レビュー）
...
`engagement-scheduled.yml`のscheduleを再導入する（cronを戻す）ことは、それ自体が別途レビュー済みの変更として扱ってください（[`docs/MANUAL_SETUP_CHECKLIST.md`](MANUAL_SETUP_CHECKLIST.md) §11）。**`config/runtime-policy.json`のManual-Only解除だけでは自動pollingは復活しません** — `allowScheduledProviderPolling:true` の設定と、cronのtriggerを`.github/workflows/engagement-scheduled.yml`のYAML自体に書く編集も同じレビュー済み変更に含める必要があります。この節の完了は「手動実行での有効化」までを指し、自動実行の再開を意味しません。
